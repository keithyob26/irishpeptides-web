...
export default function ChatPage() {
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        ...
        setMsgs(prev => [...prev, { role: 'assistant', content: err || 'Connection error.' }])
        return
      }
      ...
      if (contentType.includes('text/event-stream')) {
        ...
        const finalMsgs: Msg[] = [...newMsgs, { role: 'assistant', content: reply }]
        setMsgs(finalMsgs)
      }
    } catch {
  ...
  return (
    <div className="flex h-[calc(100vh-0px)] overflow-hidden p-8 max-w-6xl gap-0">
      {/* Sidebar */}
                    ...
                    {g.items.map(c => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 text-[#475569] hover:text-[#EF4444] text-[14px] leading-none ml-1 transition-opacity"
                          title="Delete"
                        >×</button>
                      </div>
                    ))}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="mb-5 flex items-center gap-3">
          <button
            ...
            className="text-[#64748B] hover:text-[#14B8A6] text-[18px] leading-none transition-colors"
            title={sidebarOpen ? 'Hide history' : 'Show history'}
          >☰</button>
          <PageHeader
            title="AI Chat"
            subtitle="Claude — streaming · file uploads · conversation history"
            badge={{ label: 'Claude', ok: true }}
          />
        </div>

        <div className="bg-[#1C1C1C] border border-white/[0.07] rounded-xl overflow-hidden flex flex-col flex-1">
          {/* Messages */}
          <div className="flex-1 p-6 overflow-y-auto flex flex-col gap-4 min-h-0" style={{ maxHeight: 'calc(100vh - 320px)' }}>
            {msgs.map((m, i) => (
            ...
            <div className="px-4 pt-2 flex flex-wrap gap-2">
              {files.map((f, i) => (
                <div key={i} className="flex items-center gap-1.5 bg-[#14B8A6]/10 border border-[#14B8A6]/20 rounded-lg px-2.5 py-1.5">
                  ...
                  <span className="text-[11px] text-[#94A3B8] max-w-[140px] truncate">{f.name}</span>
                  <button onClick={() => setFiles(prev => prev.filter((_, j) => j !== i))}
                    className="text-[#475569] hover:text-[#EF4444] text-[13px] leading-none ml-0.5">×</button>
                </div>
              ))}
            </div>
          )}
